...
export default function Admin() {
  ...
  const handleAddReferral = () => {
    ...
    const r = addReferral(selected.id, friendName, friendPhone);
    if (!r) {
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              ...
              {selected.referrals.length < 5 && (
                <button
                  onClick={() => setView("addReferral")}
                  className="flex items-center gap-1.5 text-xs font-body font-medium text-gold active:scale-95 transition-transform"
                >
            ...
            {selected.referrals.map((ref) => (
              <div key={ref.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/30 border border-border/30">
                ...
                {ref.validated ? (
                  <span className="text-[10px] tracking-wider uppercase font-body font-medium px-2.5 py-1 rounded-full bg-gold-light/20 text-gold-dark">
                    Validada
                  </span>
                ) : (
                  <button
        ...
        {view === "addReferral" && selected && (
          <>
            ...
            <div className="space-y-3 mt-4">
              <input
                type="text"
                placeholder="Nome da amiga indicada"
                value={friendName}
            ...
            {msg && (
              <p className={`text-xs font-body text-center ${msg.includes("!") ? "text-gold-dark" : "text-destructive"}`}>
                {msg}
              </p>
            )}
